...
public class DrivingRouteOverlay extends OverlayManager {
	...
	public DrivingRouteOverlay(BaiduMap baiduMap,LatLng mid) {
		super(baiduMap);
		this.midLatLng=mid;
	}
...
//			DrivingRouteLine.DrivingStep start=mRouteLine.getAllStep().get(0);
//			overlayOptionses.add((new MarkerOptions()).position(start.getEntrance().getLocation()).anchor(0.1f, 0.1f)
//					.zIndex(10).icon(BitmapDescriptorFactory.fromResource(R.drawable.start_point)));
//			
			for (DrivingRouteLine.DrivingStep step : mRouteLine.getAllStep()) {
				...
				if (step.getEntrance() != null) {
					overlayOptionses.add((new MarkerOptions()).position(step.getEntrance().getLocation())
							.anchor(0.01f, 0.01f).zIndex(18).rotate((360 - step.getDirection())).extraInfo(b)
...
//						&& step.getExit() != null) {
//					overlayOptionses.add((new MarkerOptions()).position(step.getExit().getLocation()).anchor(0.1f, 0.1f)
//							.zIndex(10).icon(BitmapDescriptorFactory.fromResource(R.drawable.end_point)));
...
//				}
		...
		if (mRouteLine.getAllStep() != null && mRouteLine.getAllStep().size() > 0) {
			...
			PolylineOptions option = new PolylineOptions().points(points).textureIndex(traffics).width(10)
	...
	public ArrayList<LatLng> getPointList() {
		...
		for (DrivingRouteLine.DrivingStep step : mRouteLine.getAllStep()) {
			for(int i=0;i<step.getWayPoints().size();i++){
				pointList.add(step.getWayPoints().get(i));
			}
		}
		return pointList;
	}
